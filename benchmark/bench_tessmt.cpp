...
struct HttpRequest {
    ...
    HttpRequest(size_t header_size, size_t body_size, size_t resp_size) {
        headers = (char*)allocate(header_size);
        body = (char*)allocate(body_size);
        response = (char*)allocate(resp_size);
    }
    ~HttpRequest() {
        tess_free(headers, 1024);
        tess_free(body, 4096);
        tess_free(response, 8192);
    }
};
...
int main() {
    ...
    auto end = std::chrono::steady_clock::now();
      ...
      std::cout << "TESSERACT web server (" << total_requests << " requests, " << NUM_THREADS << " threads): " 
              << elapsed << " ms\n";
